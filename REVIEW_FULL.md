...
- [ ] Персист `confidence_threshold` в SAHI.
- [ ] Персист `enabled` в TuningConfig.
- [ ] Единый ключ policy-конфига (`jobs_policy` или `jobs`) + миграция.
- [ ] Принять решение по показу Training/Detection в Jobs tab (унификация событий).
- [ ] (Опционально) убрать синхронный refresh окон при старте детекции.
