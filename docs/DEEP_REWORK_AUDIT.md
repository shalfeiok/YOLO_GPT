...
- **Minimal fix:** track `got_result` flag and raise runtime error when process exits without payload.
...
- **Regression test:** `tests/test_process_job_runner_exit_without_payload.py` ensures `JobFailed` is emitted and no `JobFinished` event is emitted.
