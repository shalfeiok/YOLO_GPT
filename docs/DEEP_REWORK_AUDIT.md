...
- **Regression test:** `tests/test_process_job_runner_exit_without_payload.py` ensures `JobFailed` is emitted when payload is missing and that late queue flush after child exit still produces `JobFinished`, and non-zero child exits without payload include exit-code diagnostics, while unknown and malformed child message envelopes (including invalid progress payload types (including non-finite NaN/inf) and out-of-range progress values normalized to [0,1]) fail explicitly instead of being silently ignored, and queue cleanup still runs when child process startup itself fails.
