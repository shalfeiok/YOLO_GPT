...
for _m in YOLO_MODEL_CHOICES:
    mid = _m.model_id
    if "yolo26" in mid:
        MODEL_HINTS[mid] = "Новейшая модель, NMS-free. Рекомендуется 100–300 эпох. Подходит для 1–1000+ классов."
    elif "yolo11" in mid:
        MODEL_HINTS[mid] = "Актуальная модель. Рекомендуется 100–300 эпох. Универсально для любого числа классов."
    elif "yolo10" in mid:
        MODEL_HINTS[mid] = "NMS-free. Рекомендуется 100–250 эпох. Хороший баланс скорости и точности."
    elif "yolov8" in mid:
        MODEL_HINTS[mid] = "Проверенная версия. Рекомендуется 100–400 эпох. Стабильный выбор для продакшена."
    elif "yolov9" in mid:
        MODEL_HINTS[mid] = "C/E — средний и большой размер. Рекомендуется 150–300 эпох. Точность выше при большем объёме данных."
    elif "yolov5" in mid:
        MODEL_HINTS[mid] = "Классика. Рекомендуется 100–500 эпох. Много классов (10–1000+) — увеличьте эпохи."
    else:
