...
def build_training_ui(view: TrainingView) -> None:
    ...
    view._apply_advisor_btn = SecondaryButton("Применить рекомендации советника")
    view._apply_advisor_btn.setEnabled(False)
    view._apply_advisor_btn.clicked.connect(view._apply_advisor_recommendations)
    view._undo_advisor_btn = SecondaryButton("Отменить применение")
    view._undo_advisor_btn.setEnabled(False)
    view._undo_advisor_btn.clicked.connect(view._undo_advisor_apply)
    btn_layout.addWidget(view._apply_advisor_btn)
    btn_layout.addWidget(view._undo_advisor_btn)
