...
class AutoAnnotationView(QWidget):
    ...
    def _build(self):
        ...
        g = QGroupBox("Автоаннотирование")
        f = QFormLayout(g)

        self._inp = QLineEdit()
        self._inp.setPlaceholderText("Папка с исходными изображениями")
        bi = QPushButton("…")
        bi.clicked.connect(self._pick_inp)
        ir = QHBoxLayout()
        ir.addWidget(self._inp, 1)
        ir.addWidget(bi)
        iw = QWidget()
        iw.setLayout(ir)
        f.addRow("Input:", iw)
        ...
        self._out = QLineEdit()
        self._out.setPlaceholderText("Папка для аннотаций")
        bo = QPushButton("…")
        bo.clicked.connect(self._pick_out)
        orr = QHBoxLayout()
        orr.addWidget(self._out, 1)
        orr.addWidget(bo)
        ow = QWidget()
        ow.setLayout(orr)
        f.addRow("Output:", ow)
        ...
        self._w.setToolTip("Путь к весам модели")
        bw = QPushButton("…")
        ...
        self._fmt = QComboBox()
        self._fmt.addItems(["YOLO TXT", "COCO JSON", "Pascal VOC XML"])
        self._fmt.setToolTip("Формат выходных аннотаций")
        f.addRow("Conf:", self._conf)
        f.addRow("IOU:", self._iou)
        f.addRow(self._only)
        f.addRow("Формат:", self._fmt)
        root.addWidget(g)

        self._run = QPushButton("Запустить аннотирование")
        self._run.clicked.connect(self._run_job)
        root.addWidget(self._run)
        self._prog = QProgressBar()
        root.addWidget(self._prog)
        self._table = QTableWidget(0, 3)
        self._table.setHorizontalHeaderLabels(["filename", "objects_found", "classes"])
        root.addWidget(self._table, 1)
        self._open = QPushButton("Открыть папку результатов")
        self._open.clicked.connect(self._open_dir)
        root.addWidget(self._open)
        self._log = QTextEdit()
        self._log.setReadOnly(True)
        root.addWidget(self._log, 1)
        self._st = QLabel("Статистика: -")
    ...
    def _pick_inp(self):
        ...
        if p:
            self._inp.setText(p)
            self._out.setText(str(Path(p).parent / f"{Path(p).name}_ann"))
    ...
    def _run_job(self):
        try:
            ...
            inp = Path(self._inp.text())
            out = Path(self._out.text())
            ...
            for i, img in enumerate(imgs, 1):
                ...
                self._prog.setValue(int(i / max(1, len(imgs)) * 100))
                self._log.append(f"{img.name}: {len(kept)} objects")
    ...
    def _save_format(self, out, img, kept, names, coco, ann_id):
        ...
        if fmt == "YOLO TXT":
            ...
            for (x1, y1, x2, y2), c, _ in kept:
                ...
                ET.SubElement(obj, "name").text = names[c]
                bb = ET.SubElement(obj, "bndbox")
                for k, v in [("xmin", x1), ("ymin", y1), ("xmax", x2), ("ymax", y2)]:
                    ET.SubElement(bb, k).text = str(int(v))
    ...
    def _open_dir(self):
        if Path(self._out.text()).exists():
            os.startfile(self._out.text())
