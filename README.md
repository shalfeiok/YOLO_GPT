...
- **Документация**: встроенный просмотр файлов из `./docs` (дерево, фильтр, markdown/text просмотр, Ctrl+F).
...
## Документация во вкладке приложения

- Все пользовательские документы храните в папке `./docs` (поддерживаются `.md`, `.txt`, `.rst`).
- Вкладка **«Документация»** автоматически сканирует `docs/` при запуске и показывает дерево файлов.
- Внутренние markdown-ссылки (на другие файлы в `docs/`) открываются внутри просмотрщика.
